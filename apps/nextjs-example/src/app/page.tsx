...
function ResultCard({ result, waiting }: { result: Result | null; waiting: boolean }) {
  ...
  const remaining =
    result?.kind === "remaining" ? result.data : null;
  ...
  return (
    <>
      ...
      <dl className="grid grid-cols-2 gap-y-2 text-sm text-zinc-600 dark:text-zinc-400">
        ...
        <dt>Reset</dt>
        <dd className="text-right font-mono">
          {reset ? <ResetCountdown reset={reset} /> : "—"}
        </dd>
      </dl>
    </>
  );
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex w-full max-w-2xl flex-col items-center gap-8 p-8">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          ...
          >
            {loading === "send" ? <Loader2 size={iconSize} className="animate-spin" /> : <Send size={iconSize} />}
            Send Request
          </button>
          <button
            onClick={() =>
          ...
          >
            {loading === "peek" ? <Loader2 size={iconSize} className="animate-spin" /> : <Eye size={iconSize} />}
            Peek Remaining
          </button>
          <button
            onClick={() =>
            ...
            className={`${btnBase} border border-zinc-300 text-black dark:border-zinc-700 dark:text-zinc-50`}
          >
            {loading === "wait" ? <Loader2 size={iconSize} className="animate-spin" /> : <Hourglass size={iconSize} />}
            Wait & Send
          </button>
        </div>
