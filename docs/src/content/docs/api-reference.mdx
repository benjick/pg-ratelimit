...
type RatelimitConfig = {
  ...
  limiter: Algorithm; // Algorithm from a static helper method
  prefix: string; // Required - namespaces keys to prevent collisions
  debug?: boolean; // Default: false - logs queries to console.debug
  clock?: Clock; // Default: () => new Date()
  cleanupProbability?: number; // Default: 0.1 - probability (0–1) of cleanup per limit() call
} & ({ durable?: false } | { durable: true; synchronousCommit?: boolean });
```

See [Database Design - synchronousCommit](/database/#synchronouscommit) for details on how this option affects write performance and durability.
